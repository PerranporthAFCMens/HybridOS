(function(){
  ...
  async function init(){
    try{
      ...
      const membership=data?.[0];if(!membership||membership.role!=='admin'||membership.access_status!=='pending')return;
      ...
      const banner=document.createElement('div');banner.id='hybridReadonlyBanner';banner.innerHTML='<b>Read-only preview</b><span>Waiting for the gym Owner to approve your Admin access.</span>';
