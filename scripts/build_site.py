...
APP_PAGES = ("index.html","classes.html","class-setup.html","admin-operations.html","resource-availability.html","staff-permissions.html","access-settings.html","reporting.html","staff.html","member.html","member-preview.html","member-memberships.html","integrations.html","social.html","onboarding.html")
TENANT_PAGES = ("index.html","member.html","member-preview.html","classes.html","staff.html","member-memberships.html","integrations.html","social.html")
ADMIN_PAGES = ("index.html","classes.html","class-setup.html","admin-operations.html","resource-availability.html","staff-permissions.html","access-settings.html","reporting.html")
def copy_source():
...
def add_shared_runtime():
    for n in APP_PAGES:
        ...
        s=inject_head(read(n),"app-consistency.css",f'<link rel="stylesheet" href="./app-consistency.css?v={VERSION}">')
        write(n,inject_head(s,"app-stability.js",f'<script src="./app-stability.js?v={VERSION}"></script>'))
def add_tenant_runtime():
    for n in TENANT_PAGES:
        if not (OUT/n).exists():continue
        s=inject_head(read(n),"tenant-branding.css",f'<link rel="stylesheet" href="./tenant-branding.css?v={VERSION}">')
        write(n,inject_body(s,"tenant-branding.js",f'<script src="./tenant-branding.js?v={VERSION}" defer></script>'))
def harden_member():
    css=f'<link rel="stylesheet" href="./member-experience.css?v={VERSION}">';js=f'<script type="module" src="./member-experience.js?v={VERSION}"></script>'
    s=read("member.html");s=s.replace("$('membershipShort').textContent=membership?.membership_plans?.name||'None';","$('membershipShort')?.textContent=membership?.membership_plans?.name||'None';",1);s=inject_head(s,"member-experience.css",css);s=inject_body(s,"social-nav.js",f'<script src="./social-nav.js?v={VERSION}" defer></script>');write("member.html",inject_body(s,"member-experience.js",js))
    s=read("member-preview.html");s=inject_head(s,"member-experience.css",css)
    ...
    write("member-preview.html",inject_body(s,"member-experience.js",js))
def add_admin_shell():
...
def add_social_runtime():
    s=read("social.html");write("social.html",inject_body(s,"social-enhancements.js",f'<script src="./social-enhancements.js?v={VERSION}" defer></script>'))
def brand_member_preview():
    ...
    write(n,s)
def build():
    copy_source();clean_legacy_mobile_chrome();add_shared_runtime();add_tenant_runtime();harden_member();add_admin_shell();add_staff_shell();add_scheduler_assets();add_social_runtime();brand_member_preview();print(f"Built Hybrid OS site in {OUT}")
