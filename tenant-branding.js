  ...
  function fixMemberPreviewClasses(){
    ...
    if(!document.getElementById('member-preview-class-modal-style')){
      ...
      style.textContent=`
        ...
        @media(max-width:700px){
          #homeClassModal.modal{place-items:end center!important;padding:0!important}
          #homeClassModal .modal-card{width:100%!important;max-width:none!important;border-radius:22px 22px 0 0!important;padding:22px 18px 28px!important;max-height:78vh!important}
        }
      `;
    ...
    document.querySelectorAll('#home .wide .row,#classes .row').forEach(function(row){
      ...
      row.onkeydown=function(e){
        if(e.key==='Enter'||e.key===' '){e.preventDefault();openClass(row);}
      };
    });

    closeBtn.onclick=closeClass;
    modal.onclick=function(e){if(e.target===modal) closeClass();};
    document.addEventListener('keydown',function(e){if(e.key==='Escape'&&!modal.classList.contains('hidden')) closeClass();});
    ...
    bookBtn.onclick=function(){
      ...
      setTimeout(function(){
        ...
        if(activeRow){
          ...
          if(!tag){tag=document.createElement('span');tag.className='tag';activeRow.querySelector('.rowtop')?.appendChild(tag);}
